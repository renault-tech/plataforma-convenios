-- Final Robust Fix for Chat Permissions (Owner & Persistence)

-- 1. Helper Function: Security Definer with FIXED search_path (Critical for security & reliability)
create or replace function public.can_access_service_chat(_service_id uuid)
returns boolean
language plpgsql
security definer
set search_path = public -- Pratica recomendada de segurança
as $$
begin
    return exists (
